import { QuizResponse, StudentStory, FacultyProfile, RAGContext } from './types';
...
export type SelectionResult = {
  student: StudentStory | null;
  faculty: FacultyProfile | null;
  alumni: StudentStory | null;
  studentScore: number;
  facultyScore: number;
  alumniScore: number;
};

export function buildMatchingProfile(params: {
  quiz: QuizResponse;
  gradeLevel: string;
  traits: string[];
  interests: string[];
...
export function selectMatchesWithMetadata(
  context: Pick<RAGContext, 'stories' | 'faculty'>,
  ...
  const studentCandidates = stories.filter((story) => story && !(story as any).classYear);
  const alumniCandidates = stories.filter((story) => story && (story as any).classYear);
  ...
  const bestStudent = bestStudentEntry?.story || (studentCandidates[0] as StudentStory | undefined) || null;
  ...
  const bestFaculty = bestFacultyEntry?.member || (faculty[0] as FacultyProfile | undefined) || null;

  let bestAlumni: StudentStory | null = null;
  let bestAlumniScore = 0;
  if (shouldIncludeAlumni(profile)) {
    const scoredAlumni = alumniCandidates
      .filter((story) => hasValidVideo((story as any).videoUrl))
      .map((story) => ({ story, score: scoreStoryRecord(story, profile) }))
      .filter((entry) => entry.score > 0)
      .sort((a, b) => b.score - a.score);

    if (scoredAlumni.length > 0) {
      bestAlumni = scoredAlumni[0].story as StudentStory;
...
export function computeCompositeScore(selection: SelectionResult): number {
  const base = 82;
...
export function summarizeMatches(selection: SelectionResult): string | null {
...
export function debugScoreFaculty(member: FacultyProfile, profile: MatchingProfile): number {
  return scoreFacultyRecord(member, profile);
}

export function debugScoreStory(story: StudentStory, profile: MatchingProfile): number {
  return scoreStoryRecord(story, profile);
}

function scoreStoryRecord(story: StudentStory, profile: MatchingProfile): number {
  let score = 12;

  const gradeBands = normalizeKeywords((story as any).gradeBands || (story.gradeLevel ? [story.gradeLevel] : []));
  score += gradeAlignmentScore(gradeBands, profile.gradeBand);

  const interestKeywords = normalizeKeywords((story as any).interestKeywords || story.interests || []);
  ...
  const personaDescriptors = normalizeKeywords((story as any).personaDescriptors || []);
  ...
  if (hasValidVideo((story as any).videoUrl)) score += 14;
  if (Array.isArray((story as any).outcomeHighlights) && (story as any).outcomeHighlights.length) score += 6;

  return score;
}

function scoreFacultyRecord(member: FacultyProfile, profile: MatchingProfile): number {
  let score = 14;

  const gradeBands = normalizeKeywords((member as any).gradeBands && (member as any).gradeBands.length ? (member as any).gradeBands : deriveGradeBandsFromTitle(member.title));
  score += gradeAlignmentScore(gradeBands, profile.gradeBand);

  const interestPool = normalizeKeywords([...(member as any).interestKeywords || [], ...(member.specializesIn || [])]);
  ...
  const personaDescriptors = normalizeKeywords((member as any).personaDescriptors || []);
  score += keywordMatchScore(personaDescriptors, profile.traits, 6, 3);

  if (hasValidVideo(member.videoUrl)) score += 18;
  if ((Array.isArray((member as any).outcomeHighlights) && (member as any).outcomeHighlights.length) || (Array.isArray(member.awards) && member.awards.length)) {
    score += 7;
  }
...
function keywordMatchScore(source: string[], targets: string[], weight: number, maxMatches = 4): number {
...
function gradeAlignmentScore(bands: string[], gradeBand: string): number {
  if (!gradeBand) return 10;
  const normalizedBands = bands.length ? bands : ['all'];
  if (normalizedBands.includes('all')) return 15;
  if (normalizedBands.includes(gradeBand)) return 24;
...
function normalizeGradeBand(gradeLevel: string): string {
  const lowerGrade = (gradeLevel || '').toLowerCase();
  if (['lower', 'elementary', 'prek-k', 'prek', 'pre-k'].includes(lowerGrade)) return 'lower';
  if (['intermediate'].includes(lowerGrade)) return 'intermediate';
  if (['middle'].includes(lowerGrade)) return 'middle';
  if (['upper', 'high', 'hs'].includes(lowerGrade)) return 'upper';
  return 'middle';
}

function deriveGradeBandsFromTitle(title: string): string[] {
  const lowerTitle = (title || '').toLowerCase();
  if (!lowerTitle) return ['all'];
  if (lowerTitle.includes('upper')) return ['upper'];
  if (lowerTitle.includes('middle') || lowerTitle.includes('6th')) return ['middle'];
  if (lowerTitle.includes('lower') || lowerTitle.includes('intermediate')) return ['lower'];
  return ['all'];
}

function shouldIncludeAlumni(profile: MatchingProfile): boolean {
...
function primaryInterestCoverageScore(source: string[], primaryInterests: string[]): number {
  if (!source.length || !primaryInterests.length) return 0;
  let score = 0;
  for (const interest of primaryInterests) {
    const synonyms = INTEREST_SYNONYMS[interest] || [interest];
    const matched = source.some((term) =>
